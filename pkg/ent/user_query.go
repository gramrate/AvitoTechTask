...
type UserQuery struct {
	...
	withAuthoredPullRequests *PullRequestQuery
	withAssignedReviews      *PullRequestQuery
	withTeam                 *TeamQuery
	withFKs                  bool
...
// QueryAuthoredPullRequests chains the current query on the "authored_pull_requests" edge.
func (_q *UserQuery) QueryAuthoredPullRequests() *PullRequestQuery {
...
// QueryAssignedReviews chains the current query on the "assigned_reviews" edge.
func (_q *UserQuery) QueryAssignedReviews() *PullRequestQuery {
	query := (&PullRequestClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withAuthoredPullRequests: _q.withAuthoredPullRequests.Clone(),
		withAssignedReviews:      _q.withAssignedReviews.Clone(),
...
// WithAuthoredPullRequests tells the query-builder to eager-load the nodes that are connected to
// the "authored_pull_requests" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithAuthoredPullRequests(opts ...func(*PullRequestQuery)) *UserQuery {
	query := (&PullRequestClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withAuthoredPullRequests = query
	return _q
}

// WithAssignedReviews tells the query-builder to eager-load the nodes that are connected to
// the "assigned_reviews" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithAssignedReviews(opts ...func(*PullRequestQuery)) *UserQuery {
	query := (&PullRequestClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withAssignedReviews = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		_spec       = _q.querySpec()
		loadedTypes = [3]bool{
			_q.withAuthoredPullRequests != nil,
			_q.withAssignedReviews != nil,
			_q.withTeam != nil,
		}
	)
	if _q.withTeam != nil {
	...
	if query := _q.withAuthoredPullRequests; query != nil {
		if err := _q.loadAuthoredPullRequests(ctx, query, nodes,
			func(n *User) { n.Edges.AuthoredPullRequests = []*PullRequest{} },
			func(n *User, e *PullRequest) { n.Edges.AuthoredPullRequests = append(n.Edges.AuthoredPullRequests, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withAssignedReviews; query != nil {
		if err := _q.loadAssignedReviews(ctx, query, nodes,
			func(n *User) { n.Edges.AssignedReviews = []*PullRequest{} },
			func(n *User, e *PullRequest) { n.Edges.AssignedReviews = append(n.Edges.AssignedReviews, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadAuthoredPullRequests(ctx context.Context, query *PullRequestQuery, nodes []*User, init func(*User), assign func(*User, *PullRequest)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(pullrequest.FieldAuthorID)
	}
	query.Where(predicate.PullRequest(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.AuthoredPullRequestsColumn), fks...))
	}))
...
func (_q *UserQuery) loadAssignedReviews(ctx context.Context, query *PullRequestQuery, nodes []*User, init func(*User), assign func(*User, *PullRequest)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.PullRequest(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.AssignedReviewsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_assigned_reviews
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_assigned_reviews" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_assigned_reviews" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
